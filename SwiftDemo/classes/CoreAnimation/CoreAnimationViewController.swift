...
class CoreAnimationViewController: UIViewController {
    ...
    func setupDataSource() {
        infos.addObject(["基础动画" : BasicAnimation.self])
        infos.addObject(["心形动画" : QTHeartAnimationViewController.self])
        infos.addObject(["心形动画2" : QTHeartAnimationController2.self])
        infos.addObject(["弹簧线" : BounceLineViewController.self])
        infos.addObject(["加载动画" : LoadingAnimationViewController.self])
        infos.addObject(["粒子喷射" : EmitterAnimationViewController.self])
    }
    
    
}

extension CoreAnimationViewController : UITableViewDelegate, UITableViewDataSource {
